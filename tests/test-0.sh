...
usage()
{
    cat << EOFU!
 Usage: $0 [flags]
Test file for pipe.pl parameter 0.
...
test()
{
    ...
    $PIPE ${params} >$out_file 2>$err_file
...
USE_CASE="Test reads file using '-0'."
INPUT_FILE=${DATA_FILE_PREFIX}.txt
PARAMETERS="-0${INPUT_FILE}"
...
USE_CASE="Test '-0' produces error when reading none existent file."
PARAMETERS="-0no_file.txt"
# Script can cause this error (OPTIONAL)
rm $EXPECTED_OUT
touch $EXPECTED_OUT
cat > $EXPECTED_ERR <<EXP_ERR!
No such file or directory at ../pipe.pl line 4108.
EXP_ERR!
