...
usage()
{
    cat << EOFU!
 Usage: $0 [flags]
Test file for pipe.pl parameter o.
...
USE_CASE="Test output specific column (c1)."
PARAMETERS="-oc1"
...
USE_CASE="Test 'remaining' keyword:output all remaining columns in order."
PARAMETERS="-oc1,remaining"
INPUT_FILE=${DATA_FILE_PREFIX}.txt
EXPECTED_OUT=${EXPECTED_STDOUT}.0.txt
EXPECTED_ERR=${EXPECTED_STDERR}.0.txt
## Create some data to pipe into $PIPE 
cat >$INPUT_FILE <<FILE_DATA!
21221000000000|One col|
21221011111111|One col|Two col|
21221022222222|One col|Two col|Three col|
21221033333333|One col|Two col|Three col|Four col|
FILE_DATA!
# Expected: error message issued.
cat > $EXPECTED_OUT <<EXP_OUT!
One col|21221000000000
One col|21221011111111|Two col
One col|21221022222222|Two col|Three col
One col|21221033333333|Two col|Three col|Four col
EXP_OUT!
...
USE_CASE="Test 'continue' keyword: output c1 and all others to EOL."
...
USE_CASE="Test 'last' keyword: output c0 and last column on line."
PARAMETERS="-oc0,last"
INPUT_FILE=${DATA_FILE_PREFIX}.txt
EXPECTED_OUT=${EXPECTED_STDOUT}.0.txt
EXPECTED_ERR=${EXPECTED_STDERR}.0.txt
...
cat > $EXPECTED_OUT <<EXP_OUT!
21221000000000|One col
21221011111111|Two col
21221022222222|Three col
21221033333333|Four col
EXP_OUT!
...
USE_CASE="Test 'reverse' keyword: output columns in reverse order."
PARAMETERS="-oreverse"
INPUT_FILE=${DATA_FILE_PREFIX}.txt
EXPECTED_OUT=${EXPECTED_STDOUT}.0.txt
EXPECTED_ERR=${EXPECTED_STDERR}.0.txt
...
cat > $EXPECTED_OUT <<EXP_OUT!
One col|21221000000000
Two col|One col|21221011111111
Three col|Two col|One col|21221022222222
Four col|Three col|Two col|One col|21221033333333
EXP_OUT!
...
USE_CASE="Test 'exclude' keyword: output all columns except c2,c3."
PARAMETERS="-oc2,c3,exclude"
INPUT_FILE=${DATA_FILE_PREFIX}.txt
EXPECTED_OUT=${EXPECTED_STDOUT}.0.txt
EXPECTED_ERR=${EXPECTED_STDERR}.0.txt
## Create some data to pipe into $PIPE 
...
cat > $EXPECTED_OUT <<EXP_OUT!
21221000000000|One col
21221011111111|One col
21221022222222|One col
21221033333333|One col|Four col
EXP_OUT!
