...
usage()
{
    cat << EOFU!
 Usage: $0 [flags]
Test file for pipe.pl parameter '-d'.
...
USE_CASE='Test if -d deduplicates text.'
PARAMETERS='-dc0'
### Use case starts
...
((TEST_NUMBER++))
### Use case ends
USE_CASE='Test if -A deduplicates outputs the count of duplicates.'
PARAMETERS='-dc0 -A'
### Use case starts
INPUT_FILE=${DATA_FILE_PREFIX}.$TEST_NUMBER.txt
## Create input data $PIPE 
