...
#
# Rev:
# 0.46.05 - Feb 1, 2018 Add 'any' keyword to -o function.
#
####################################################################################
...
All column references are 0 based. Line numbers start at 1.
 ...
 -o{any|c0,c1,...cn}: Order the columns in a different order. Only the specified columns are output, unless.
                  the keyword 'any' is used, then the remaining columns are output in left-to-right order.
...
sub order_line( $ )
{
	...
	foreach my $c ( @ORDER_COLUMNS )
	{
		...
		last if ( $c =~ m/($KEYWORD_ANY)/i );
		unshift @order_columns, $c;
	}
	if ( grep /($KEYWORD_ANY)/i, @ORDER_COLUMNS )
	{
		foreach my $colIndex ( 0 .. scalar( @{ $line } ) -1 )
		{
			next if ( grep /($colIndex)/, @order_columns );
			unshift @order_columns, $colIndex;
		}
	}
...
sub init
{
	...
	@ORDER_COLUMNS     = read_requested_columns( $opt{'o'}, $KEYWORD_ANY )                  if ( $opt{'o'} );
